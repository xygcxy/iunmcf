    ...
    if ( ! class_exists( 'ReduxFramework_import_export' ) ) {
        ...
        class ReduxFramework_import_export extends ReduxFramework {
            ...
            public function render() {
                ...
                <p><a href="javascript:void(0);" id="redux-import-code-button" class="button-secondary"><?php _e( '导入文件', 'redux-framework' ); ?></a> <a href="javascript:void(0);" id="redux-import-link-button" class="button-secondary"><?php _e( 'Import from URL', 'redux-framework' ) ?></a></p>
                ...
                <p id="redux-import-action"><input type="submit" id="redux-import" name="' . $this->parent->args['opt_name'] . '[import]" class="button-primary" value="<?php _e( 'Import', 'redux-framework' ) ?>">&nbsp;&nbsp;<span><?php echo apply_filters( 'redux-import-warning', __( '注意! 导入操作会导致现有设置内容全部被覆盖，请先备份!', 'redux-framework' ) ) ?></span></p>

                <div class="hr"/><div class="inner"><span>&nbsp;</span></div></div>
                <h4><?php _e( 'Export Options', 'redux-framework' ) ?></h4>
                <div class="redux-section-desc">
                <p class="description"><?php echo apply_filters( 'redux-backup-description', __( '您可以导出现有设置，到其他站点进行导入.', 'redux-framework' ) ) ?></p>
                </div>
                <?php
                $link = admin_url( 'admin-ajax.php?action=redux_download_options-' . $this->parent->args['opt_name'] . '&secret=' . $secret );
                ?>
                <p><a href="javascript:void(0);" id="redux-export-code-copy" class="button-secondary"><?php _e( '复制现有数据', 'redux-framework' ) ?></a> <a href="<?php echo $link; ?>" id="redux-export-code-dl" class="button-primary"><?php _e( '将文件导出并下载', 'redux-framework' ) ?></a> <a href="javascript:void(0);" id="redux-export-link" class="button-secondary"><?php _e( '复制导出文件地址', 'redux-framework' ) ?></a></p>
                <p></p>
